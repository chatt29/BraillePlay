...
public class MainMenuControl : MonoBehaviour
{
    ...
    [TextArea(2, 4)]
    public string instructionSpeech =
        "Use up and down to choose an option. Press submit to select.";
    ...
    private void SelectOption(int index, bool speak)
    {
        ...
        if (options[currentIndex].button != null && EventSystem.current != null)
        {
            EventSystem.current.SetSelectedGameObject(
                options[currentIndex].button.gameObject);
        }
    ...
    private void SelectCurrentOption()
    {
        ...
        {
            Debug.LogWarning(
                "No scene name assigned for selected menu option.");
        }
    }
