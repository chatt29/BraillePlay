...
public class BrailleSoundsAround1 : MonoBehaviour
{
    ...
    public string highScoreKey = "CConQuestHighScore";
    ...
    private bool waitingForSoundQuestion;
    ...
    private void AddMistake()
    {
        ...
        // Deduct score only every 3 mistakes
        int deductions = totalWrongCount / 3;

        totalScore = Mathf.Max(
            0,
            fixedScore - (deductions * deductionPerMistake)
        );
    ...
    private IEnumerator StartLessonSequence(BrailleLesson lesson)
    {
        ...
        if (lesson.useSoundQuestion)
        {
            ...
            // 2. Sound effect
            if (lesson.soundEffectAudio != null)
            {
                voiceAudioSource.Stop();
            ...
            yield break; // IMPORTANT: stop normal lesson flow here
        }
    ...
    private void HandleSoundQuestion(string pattern)
    {
        ...
        if (pattern == "100000") // Dot 1
            userAnswer = true;
        else if (pattern == "010000") // Dot 2
            userAnswer = false;
        else
            return;
        ...
        if (userAnswer == lesson.correctAnswerIsTrue)
        {
            ...
            flowRoutine = StartCoroutine(ContinueAfterSoundQuestion(lesson));
        }
        else
        {
            AddMistake();
    ...
    private void AddTypedCharacterFromPattern(string pattern)
    {
        ...
        int stepIndex = currentSequenceStep;
        ...
        string valueToAdd = lesson.expectedSequenceNames[stepIndex];

        if (string.IsNullOrEmpty(valueToAdd))
            return;

        // If it's "space", insert actual space
        if (valueToAdd.ToLower() == "space")
            typedSentence += " ";
        else
            typedSentence += valueToAdd;
    ...
    private IEnumerator ContinueAfterSoundQuestion(BrailleLesson lesson)
    {
        yield return ShowBubbleMessageSynced(
            "Correct!",
            genericCorrectAudio,
